...
/**
 * Standalone logger for SGCServer. Mirrors LumiBot's logger with an [SGC]
 * prefix so split-process logs can be grepped/teed easily.
 *
 * Set LOG_LEVEL=debug in SGCServer/.env to see verbose split-debug output.
 */
...
function write(level, message, ...meta) {
  ...
  const output = meta.length > 0
    ...
    : message;
  const line = `[${new Date().toISOString()}] [SGC] [${level.toUpperCase()}] ${output}`;
